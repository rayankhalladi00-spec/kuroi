// Demon Slayer: Kimetsu no Yaiba — titres relevés sur la Wikipédia francophone.
// Les résumés viendront plus tard, rédigés pour ce site.

const meta = {
  title: 'Demon Slayer',
  year: 2019,
  genre: 'Aventure',
  hue: 15,
  description:
    "Sa famille massacrée et sa sœur transformée en démon, un jeune garçon s'engage " +
    "chez les pourfendeurs pour lui rendre son humanité.",
};

const titles = {
  1: [
    'Cruauté',
    'Urokodaki Sakonji, le formateur',
    'Sabito et Makomo',
    'La Sélection finale',
    'Chacun son acier',
    'Le chasseur accompagné d’un démon',
    'Kibutsuji Muzan',
    'Le Parfum du sang d’envoûtement',
    'Les Démons aux ballons et aux flèches',
    'Nous resterons ensemble',
    'La Maison aux tambours',
    'Zenitsu dort, le sanglier montre les crocs',
    'Ce à quoi je tiens plus qu’à ma vie',
    'Le Blason des glycines',
    'Le Mont Natagumo',
    'Faire passer les autres avant soi',
    'Parfaire une seule technique',
    'Des liens factices',
    'Le Dieu du feu',
    'Une famille improvisée',
    'Violation du code',
    'Le Maître',
    'La Réunion des piliers',
    'L’Entraînement récupérateur',
    'Tsuyuri Kanao, la successeuse',
    'Nouvelle Mission',
  ],
  2: [
    'Rengoku Kyôjurô, le pilier de la flamme',
    'Un profond sommeil',
    'Si c’était réel',
    'Affront',
    'À l’avant du train',
    'Akaza',
    'Enflamme ton âme',
    'Uzui Tengen, le pilier du son',
    'Infiltration au quartier des plaisirs',
    'Qui es-tu ?',
    'Cette nuit',
    'Combattre avec panache',
    'Des souvenirs accumulés',
    'Métamorphose',
    'Union',
    'Si on abat une lune supérieure',
    'Je n’abandonnerai jamais',
    'À chaque réincarnation',
  ],
  3: [
    'Le Rêve d’un étranger',
    'Yoriichi modèle zéro',
    'Le Katana de plus de 300 ans',
    'Merci, Tokitô',
    'Le Sabre écarlate',
    'Tu veux devenir un pilier, non ?',
    'Les Véritables Monstres',
    'Le Sens du prénom de Muichirô',
    'Tokitô Muichirô, le pilier de la brume',
    'Kanroji Mitsuri, le pilier de l’amour',
    'Unis au point du jour',
  ],
  4: [
    'Pour vaincre Kibutsuji Muzan',
    'La Souffrance de Tomioka Giyû, pilier de l’eau',
    'Tanjiro est rétabli et se joint à l’entraînement',
    'Souris',
    'Manger des démons',
    'Le Plus Fort des pourfendeurs',
    'Himejima Gyômei, pilier du rocher',
    'Rassemblement des piliers',
  ],
};

const synopses = {};

module.exports = { meta, titles, synopses };
