...
// Images d'episode. Dans data/ comme le reste : le service n'a le droit
// d'ecrire que la (ProtectSystem=strict).
const EPISODE_DIR = path.join(DATA_DIR, 'episodes');
fs.mkdirSync(EPISODE_DIR, { recursive: true });
const MAX_EPISODE_SIZE = 3 * 1024 * 1024; // 3 Mo : une capture, pas une affiche

const episodeStorage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, EPISODE_DIR),
  filename: (req, file, cb) => {
    const ext = path.extname(file.originalname).toLowerCase();
    cb(null, crypto.randomBytes(8).toString('hex') + ext);
  },
...
const uploadEpisodeImage = multer({
  storage: episodeStorage,
  limits: { fileSize: MAX_EPISODE_SIZE, files: 1 },
  fileFilter: (req, file, cb) => {
    const ext = path.extname(file.originalname).toLowerCase();
    // Pas de SVG : il peut porter du script, et serait servi depuis notre
    // propre domaine.
    if (!['.png', '.jpg', '.jpeg', '.webp', '.avif', '.gif'].includes(ext))
      return cb(new Error('Formats acceptés : png, jpg, webp, avif, gif.'));
    cb(null, true);
